...
// Final Class
final class ArtMuseum {

	// Theme Version
	private $artmuseum_version = '1.0';
	...
	private function setup() {
		...
		$enqueu = new ArtMuseum_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->artmuseum_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'artmuseum_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'art-museum', ARTMUSEUM_DIR_PATH . '/languages' );
		...
		add_image_size( 'artmuseum_blogsect_post_thumb', 295, 211, true );
		
		// Custom thumbnails size for widget post
		add_image_size( 'artmuseum_widget_post_thumb', 75, 75, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'artmuseum-np-thumb', 59, 59, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'art-museum' ),
            'social-menu'    => esc_html__( 'Social Menu', 'art-museum' ),
        ) );
	...
	private function enqueue() {
		...
		$cssPath = ARTMUSEUM_DIR_CSS_URI;
		$jsPath  = ARTMUSEUM_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					...
					'dependency' 	=> array(),
					'version' 		=> $this->artmuseum_version,
				),
				array(
					'handler'		=> 'artmuseum-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->artmuseum_version,
				),
				array(
					'handler'		=> 'artmuseum-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'art-museum' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate artmuseum theme customizer
		$artmuseum_theme_customizer = new artmuseum_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'artmuseum_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'artmuseum_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'artmuseum-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'artmuseum_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'artmuseum-elementor-default-style',  ARTMUSEUM_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->artmuseum_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'artmuseum_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'artmuseum-elementor-notice', ARTMUSEUM_DIR_JS_URI.'artmuseum-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'artmuseum-elementor-notice',
				'artmuseumElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'artmuseum-elementor-notice-nonce' ),
				)
			);
		}
