...
add_action( 'wp_enqueue_scripts', 'artmuseum_sl_enqueue_scripts' );
function artmuseum_sl_enqueue_scripts() {

	wp_enqueue_script( 'artmuseum-post-likes', ARTMUSEUM_COMPANION_DIR_URL. 'js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'artmuseum-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'artmuseum-core' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_artmuseum_process_simple_like', 'artmuseum_process_simple_like' );
add_action( 'wp_ajax_artmuseum_process_simple_like', 'artmuseum_process_simple_like' );
function artmuseum_process_simple_like() {
	// Security
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'artmuseum-core' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'artmuseum-core' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'artmuseum-core' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'artmuseum-core' );
...
// User Profile List
add_action( 'show_user_profile', 'artmuseum_show_user_likes' );
add_action( 'edit_user_profile', 'artmuseum_show_user_likes' );
function artmuseum_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'artmuseum-core' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'artmuseum-core' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
